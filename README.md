...
The doc app itself is a client app of the libary, therefore PrimeVue is imported in [docs/.vitepress/theme/index.ts](docs/.vitepress/theme/index.ts). The configuration in [docs/.vitepress/config.ts](docs/.vitepress/config.ts) below forces VitePress to resolve these modules with no duplication, avoiding error at runtime, as PrimeVue also has Vue in its dependency.
